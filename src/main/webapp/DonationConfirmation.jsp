...
<body>
         <header> <h2>Thank You For Donating <span><i class='bx bxs-donate-blood' ></i></span></h2></header>	
 <section class="cta" id="cta">
 		<% try {
                 Connection connection = CommonConnection.getConnection();
                 Statement st = connection.createStatement();
                 ResultSet rs = st.executeQuery("select did , dname from donor ORDER BY did DESC LIMIT 1");
                 while (rs.next()) {
         %>
 	  <h3>Donor ID: <%= rs.getInt(1) %></h3>
      <h2>Donor Name: <%= rs.getString(2) %></h2>
